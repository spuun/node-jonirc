...
var irc = function(server, nick, config) {
	...
	client.on('001', (servername, nick) => {
		log.debug(`Connected to ${servername} as ${nick}`);
		connection.nick = nick;
		emit('connected', {server: servername});
	});
	client.on('nick', (userinfo, newNick) => {
		var uinfo = parseUserinfo(userinfo); 
		if (uinfo.nick == connection.nick) {
			var oldNick = uinfo.nick;
			connection.nick = newNick;
			log(`Bot changed nick to ${connection.nick}`);
			emit('botnick', oldNick, newNick);
		}
	});
	...
	var commandRegex = new RegExp('^'+config.commandPrefix+'(\\w+) ?(.*?)(?:\\s*'+config.commandPipe+'\\s*(\\w+)\\s*)?$');
	...
	// Support for commands via msg to bot
	var botCommandRegex = new RegExp('^'+config.commandPrefix+'(\\w+) ?(.*?)$');
	client.on('botmsg', (userinfo, message) => {
		var matches = botCommandRegex.exec(message);
		if (matches == null) {
			return;
		}
		var uinfo = parseUserinfo(userinfo);
		...
		var commandArguments = matches[2];
		var respond = (msg) => {
			client.send(`PRIVMSG ${uinfo.nick} :${msg}`);
		}
		emit('botcmd', userinfo, command, commandArguments, respond);
		emit('botcmd:'+command, userinfo, commandArguments, respond);
	});
